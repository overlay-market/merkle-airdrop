  ...
  async process(): Promise<void> {
    ...
    for (let i = 0; i < this.recipients.length; i += batchSize) {
      ...
      await prisma.user.createMany({
        data: batch.map(({ address, value }) => ({
          wallet: address,
          contract: process.env.AIRDROP_CONTRACT_ADDRESS || "",
          tokenAmount: value,
          proof: merkleTree
            .getProof(this.generateLeaf(address, value))
            .map((proof) => {
              return proof.data.toString("hex")
            }),
        })),
      })
      logger.info(`Batch ${i / batchSize} inserted to DB.`)
    }

    logger.info("All batches inserted to DB.")
